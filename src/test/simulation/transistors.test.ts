...
/**
 * Find the DIST gain control in the MXR Distortion+ preset.
 *
 * Example node ids are generated by the editor and are not stable across
 * saves. The control label is the durable identifier that matters for these
 * tests because the simulation behavior depends on the pedal's named pot, not
 * on a specific UUID in the JSON fixture.
 */
function findDistortionPlusDistPot(
  nodes: Array<ComponentNode>,
): Extract<ComponentNode, { type: 'pot' }> {
  for (const node of nodes) {
    if (node.type !== 'pot') {
      continue;
    }

    if (node.data.label !== 'DIST') {
      continue;
    }
...
describe('MXR Distortion+ preset (LM741 + 1N270)', () => {
  ...
  it('lower DIST positions increase gain across the feedback path', async () => {
    const distortionPlus = await import(
      '../../examples/pedals/distortion-plus.json'
    );
    ...
    const highGainPot = findDistortionPlusDistPot(highGainNodes);
    const lowGainPot = findDistortionPlusDistPot(lowGainNodes);
    ...
    highGainPot.data.position = 0;
    lowGainPot.data.position = 1;
  ...
  it('default DIST setting is quieter than a high-drive setting', async () => {
    const distortionPlus = await import(
      '../../examples/pedals/distortion-plus.json'
    );
    ...
    const highDriveNodes = structuredClone(
      distortionPlus.nodes,
    ) as Array<ComponentNode>;

    findDistortionPlusDistPot(defaultDriveNodes);
    const highDrivePot = findDistortionPlusDistPot(highDriveNodes);

    const defaultDriveCircuit = makeCircuit(
      defaultDriveNodes,
      distortionPlus.edges as Array<Edge>,
    );
    const highDriveCircuit = makeCircuit(
      highDriveNodes,
      distortionPlus.edges as Array<Edge>,
    );

    highDrivePot.data.position = 0;
    ...
    const highDriveNetlist = compileNetlist(
      highDriveCircuit.nodes,
      highDriveCircuit.edges,
      0.01,
      1000,
      0.1,
    );
    ...
    const highDriveOutput = await engine.run(highDriveNetlist);

    const defaultDrivePeak = peak(defaultDriveOutput.voltageValues, 0.2);
    const highDrivePeak = peak(highDriveOutput.voltageValues, 0.2);

    expect(highDrivePeak).toBeGreaterThan(defaultDrivePeak * 1.5);
  });
});
