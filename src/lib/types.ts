...
export type OpAmpData = { label: string; model: 'TL072' | 'LM741' | 'LM308'; symbol?: string };
export type PowerData = { label: string; volts: number; symbol?: string };
export type GroundData = { label: string; symbol?: string };
export type JackData = { label: string; direction: 'in' | 'out'; symbol?: string };
export type DiodeData = { label: string; model: '1N914' | '1N4001' | '1N270'; symbol?: string };
...
export type BJTModel = '2N3904' | '2N3906' | 'AC128' | '2N5088' | '2N5089' | 'BC108' | 'BC549' | 'MPSA18';
export type BJTData = {
  label: string;
  polarity: 'NPN' | 'PNP';
  model: BJTModel;
...
export type ComponentNode =
  ...
  | (NodeBase & { type: 'box'; data: BoxData; style?: { width?: number; height?: number }; dragHandle?: string; className?: string });
