 ...
 *
 * These types describe the structure of a schematic symbol: how it looks on
 * the canvas, where its pins live, and what electrical role each pin serves.
 */

/**
 * Electrical category of a pin.
 *
 * Drives visual style cues (colour, dash pattern) and future DRC (design rule
 * check) hints.
 *
 ...
 *
 * Pins map directly to XYFlow handles. The `id` field must match the handle
 * IDs expected by the netlist compiler so that wiring produces valid SPICE
 * nets.
 *
...
export type SymbolPin = {
  ...
  /**
   * Functional label shown to the user, for example IN+, V-, or OUT.
   */
  name: string;
  ...
  /**
   * Pixel offset along the edge (from top for left/right sides, from left for
   * top/bottom sides). Use '50%' to centre on the edge.
   */
  offset: number | '50%';
 ...
 *
 * Node renderers switch on this value to choose the appropriate SVG drawing.
 * Pin data (SymbolPin) drives handle placement for every style.
 *
 ...
 *
 * Each component's `data.symbol` field names a SymbolDef by its `id`. When
 * absent, DEFAULT_SYMBOL in the index resolves the appropriate default for the
 * component's model.
 */
export type SymbolDef = {
  /**
   * Unique library identifier, for example `opamp/lm308-dip8`.
   */
  id: string;
  ...
  /**
   * Complete pin list, including connectable and decorative pins.
   */
