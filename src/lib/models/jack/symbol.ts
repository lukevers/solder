 ...
 *
 * The netlist compiler creates a voltage source (`Vin`) between `pos` and
 * `neg` for each input jack.
 *
 *   ┌──────┐
 *   │ IN   ├── pos (signal)
 *   │      ├── neg (ground)
 ...
 *
 * The netlist compiler measures voltage between `pos` and `neg` to produce the
 * output buffer.
 *
 *                  ┌──────┐
 *   pos (signal) ──┤      │
