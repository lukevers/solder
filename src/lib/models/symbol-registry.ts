 ...
 *
 * This module contains only static symbol
 * definitions and lookup helpers. It must stay
 * free of React node renderers and store imports
 * so it can be shared by workers, store helpers,
 * and renderer code without creating cycles.
 */

import { SYM_BJT_NPN, SYM_BJT_PNP } from './bjt/symbol';
...
import { SYM_JFET_N, SYM_JFET_P } from './jfet/symbol';
import { SYM_MOSFET_N, SYM_MOSFET_P } from './mosfet/symbol';
import {
  SYM_LM308_DIP8,
  SYM_LM741_DIP8,
  SYM_OPAMP_TRIANGLE,
 ...
 *
 * Store helpers use this to inject `measured`
 * dimensions before XYFlow performs its first
 ...
 *
 * The store uses this when a circuit JSON blob
 * omits `data.symbol`, and the inspector uses it
 * to pick the right initial choice for a model.
 */
 ...
 *
 * The fallback chain keeps older saved circuits
 * working even if they omit a symbol ID or point
